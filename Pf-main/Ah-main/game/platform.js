(function () {
  ...
  function setupGameShell() {
    ...
    document.addEventListener('pointerdown', event => {
      rightJoystickActive = Boolean(event.target.closest && event.target.closest('#joy-right'));
    }, true);
    document.addEventListener('pointerup', event => {
      if (rightJoystickActive) setTimeout(() => { rightJoystickActive = false; }, 0);
    }, true);
    document.addEventListener('pointercancel', () => { rightJoystickActive = false; }, true);
    const placeBuilding = window.tryPlaceBuilding;
    if (typeof placeBuilding === 'function') {
      window.tryPlaceBuilding = function guardedPlaceBuilding(...args) {
        if (rightJoystickActive) return false;
        return placeBuilding.apply(this, args);
      };
    }
